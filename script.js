...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /**
   * Formatea una fecha en el formato dd/mm/aa.
   * @param {string} dateString - La fecha en formato ISO (yyyy-mm-dd).
   * @returns {string} La fecha formateada como dd/mm/aa.
   */
  ...
  /**
   * Convierte una fecha en formato dd/mm/aa a formato ISO (yyyy-mm-dd).
   * @param {string} formattedDate - La fecha en formato dd/mm/aa.
   * @returns {string} La fecha en formato ISO.
   */
  function parseDate(formattedDate) {
    const [day, month, year] = formattedDate.split("/");
    return `20${year}-${month}-${day}`; // Convierte a formato ISO
  }
  ...
  function calculateTotal() {
    ...
    const total = notes.reduce(
      (sum, note) => sum + parseFloat(note.monto || 0),
      0
    );
  ...
  function renderNotes() {
    ...
    if (notes.length === 0) {
      ...
      notes.forEach((note) => {
        ...
        row.innerHTML = `
                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">${formatDate(
                      note.fecha
                    )}</td>
                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">${
                      note.concepto
                    }</td>
                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 text-right">${formatCurrency(
                      note.monto
                    )}</td>
                    <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-center space-x-2">
                        <button class="action-button edit-btn text-blue-600 hover:text-blue-800" data-id="${
                          note.id
                        }" title="Editar">
                            <i data-lucide="edit-2" class="w-4 h-4 sm:w-5 sm:h-5"></i>
                        </button>
                        <button class="action-button delete-btn text-red-600 hover:text-red-800" data-id="${
                          note.id
                        }" title="Borrar">
  ...
  function showModal(isEditing = false, noteData = null) {
    ...
    modalTitle.textContent = isEditing
      ? "Editar Apunte"
      : "Añadir Nuevo Apunte"; // Cambia el título del modal

    if (isEditing && noteData) {
      // Si se está editando, rellena el formulario con los datos existentes
      noteIdInput.value = noteData.id;
      fechaInput.value = formatDate(noteData.fecha); // Muestra la fecha formateada
  ...
  function handleFormSubmit(event) {
    ...
    const formData = {
      ...
      fecha: parseDate(fechaInput.value), // Convierte la fecha al formato ISO
